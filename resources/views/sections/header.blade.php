<header class="header flex flex-row justify-between items-center p-5 bg-white shadow-sm">
  ...
  <div class="nav">
    ...
    <div x-data="{ open: false }"  class="nav--mobile">
      <div x-on:click="open = ! open"  class="hamburger relative h-[18px] w-[30px]">
        <span class="hamburger-barOne bg-black absolute rounded h-[3px] w-[30px] top-[0px]"></span>
        <span class="hamburger-barTwoo bg-black absolute rounded h-[3px] w-[30px] top-[9px]"></span>
        <span class="hamburger-barThree bg-black absolute rounded h-[3px] w-[20px] top-[18px]"></span>
      </div>
      <nav x-show="open" class="nav-primary bg-white text-black text-center" aria-label="{{ wp_get_nav_menu_name('primary_navigation') }}">
        {!! wp_nav_menu(['theme_location' => 'primary_navigation', 'menu_class' => 'nav', 'items_wrap' => '<ul class="nav your_custom_class">%3$s</ul>', 'echo' => false]) !!}
      </nav>
    </div>
  </div>
